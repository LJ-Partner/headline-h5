        ...
        <v-main v-bind:no-padding="true" v-bind:isbg="true">
            <form>
                <ul class="user-nickname">
                    <li>
                        <input type="text" value="{{forms.position}}" maxlength="10" v-model="forms.position" />
                    </li>
                </ul>
               <div class="usernickname-subwrap">
                    <button type="button" class="btn-sub" v-on:click="reset" :disabled="btn.disabled || !forms.position">{{btn.text}}</button>
                </div>
            </form>
        </v-main>
        <v-footer></v-footer>
...
export default {
        ...
        mounted () {
            this.$store.dispatch('set_usercenter_title','修改公司职位');
            this.$store.dispatch('set_channel_cur','user');
            if(user_interest){
                this.forms.position = user_interest.position;
            }
        },
        methods: {
           reset () {
                if(this.forms.position == "") {
                    this.show_dialog("请输入您的职位",true,2000,false);
                } else {
                    this.btn.disabled=true;
                    this.btn.text="修改中..";
                    this.$http.post(API.updateUserInfo,{
                        user_id:parseInt(user.user_id),
                        content:this.forms.position,
                        user_phone:user.user_phone,
                        userRegistInfoEnum:2                                                //修改的类别 1所属企业 2职位 3 身份
                    })
                    .then((response)=>{
                        // this.loading = false;
                        ...
                        if(response.data.code=="500"){
                            ...
                            if(user_interest){
                                user_interest.position = this.forms.position;
                                Storage.set('ljh5user_interest', user_interest);
                            }else{
                                Storage.set('ljh5user_interest', {position:this.forms.position});   
                            }
