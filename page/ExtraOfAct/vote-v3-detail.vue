<template>
    <v-content>
        ...
        <div class="vote-v3-detail" v-show="!loading" v-if="contFlag">
            <div class="vote-detail-main" id="mainWrap">
                <article class="vote-detail">
                    <a class="covel" v-if="comDetail.icon_uri"><img :src="comDetail.icon_uri"> </a>
                ...
                <div class="vote-comment" id="voteComment">
                    ...
                    <div class="no-comment" v-else>
                        <img src="https://p.maicai360.cn/img/get/20180209/08320636537961891158866_png" />
                        <p>来了？请您坐沙发</p>
                    </div>-->
                </div>
            </div>
                ...
                <div class="detail-mask-wrap" v-if="showMask == 1">
                    <div class="pop-w">
                        <div class="pop">
                            ...
                            <div class="pop-cnt">
                                <img src="http://p.maicai360.cn/img/get/20171031/25281636450584063881573_jpg" class="ewm"  v-if="activityId == '6'">
                                <img src="http://p.maicai360.cn/img/get/20171124/78171636471365373251613_jpg" class="ewm"  v-if="activityId == '9'">
                                <img :src="QrCodeUri" class="ewm" v-else-if="QrCodeUri">   
                                <img src="https://p.maicai360.cn/img/get/20180209/64704636537962145359740_png" class="ewm" v-else> 
                                <ul class="left">
                                    <li v-if="activityId == '6' ">电话：<a href="tel:13162353128">131-6235-3128</a> 旺材小编 </li>
                                    <li v-else-if="activityId == '7' ">电话：<a href="tel:15801718349">158-0171-8349</a> 旺材小编 </li>
                                    <li v-else-if="activityId == '8' || activityId == '9'">电话：<a href="tel:17601245905">176-0124-5905</a> 旺材小编 </li>
                                    <li v-else>电话：<a href="tel:18017160592">180-1716-0592</a>旺材小编</li>
                                    <li v-if="activityId == '6' || activityId == '7'">微信：huganqi_bianji</li>
...
export default {
    data(){
        return {
            ...
            shareData:{     //微信分享默认内容
                ...
                link: 'https://m.maicai360.cn'+this.$route.path+'?companyId='+this.$route.query.companyId+'&activityId='+this.$route.query.activityId
            }
        }    
    },
    mounted() {
        ...
        if(this.$route.query.meetingid && this.$route.query.meetingid != 'undefined'){
            this.meetingid = this.$route.query.meetingid;
            this.shareData.link = 'https://m.maicai360.cn'+this.$route.path+'?companyId='+this.$route.query.companyId+'&activityId='+this.$route.query.activityId+'&meetingid='+this.meetingid;
        }else{
            this.meetingid = 1;    
        }
    ...
    methods:{
        ...
        //获取公司详情
        getCompanyInfo(){
            this.$http.get(API.getVoteComDetail,{params:{activityId:this.activityId,companyId:this.companyId,userid:this.user.user_id || 0}})
            .then((response)=>{
                if(response.data.code && response.data.code=="200"){
                    this.comDetail = response.data.data;
                    this.canVote = this.comDetail.can_vote;
                    this.groupId = this.comDetail.group_id;
                    let _url;
                    if(this.$route.query.meetingid && this.$route.query.meetingid != 'undefined'){
                        _url = '&meetingid=' + this.$route.query.meetingid;
                    }else{
                        _url = '&meetingid=' + this.meetingid;
                    }
                    this.footItems[0].link = '/ExtraOfAct/VoteVthree?groupId='+this.groupId+_url;
                    this.footItems[1].link = '/ExtraOfAct/VoteVthree?groupId='+this.groupId+'&footIndex=1'+_url;
                    this.footItems[2].link = '/ExtraOfAct/VoteVthree?groupId='+this.groupId+'&footIndex=2'+_url;
                    this.comDetail.comments = this.comDetail.comments  != null ? this.comDetail.comments : [];//评论列表
                    if(this.comDetail.company_name){ 
                        if(this.meetingid == 1){
                            this.shareData.title = '投我一票 | (' + this.comDetail.company_name + ')中国电机控制器优质适配能力优选';
                            this.shareData.desc = '我的未来 由你决定，每日每人10票，请为我投票';
                        }else{
                            this.shareData.title = '投我一票 | (' + this.comDetail.company_name + ')中国电机控制器优质适配能力评选';
                            this.shareData.desc = '中国电机控制器优质适配能力评选';
                        }
                    }
        ...
        toVote(){
            let that = this;
            if(that.user){
                let encrypted = CryptoJS.AES.encrypt((this.user.user_phone+'_' + this.companyId + '_' + this.activityId), Key,{
                    keySize: 128 / 8,
                    iv: Iv,
                ...
                if(that.canVote !== false){
                    if(that.isDisabled){
                        that.$http.post(API.getToVote,{user_phone:this.user.user_phone,company_id:this.companyId,activity_id:this.activityId,cipher_text:encrypted.toString()})
                        .then((response)=>{
                            if(response.data.code && response.data.code=="200"){
        ...
        toComment(){
            ...
            if(this.user){
                if(this.user_comment != "" && this.user_comment.length<200){
                    this.$http.post(API.voteToComment,{voting_id:this.activityId,company_id:this.companyId,userid:this.user.user_id,comments:this.user_comment})
                    .then((response)=>{
                        if (response.data.code && response.data.code == '200') {
                        ...
                        　　　　　　 mainWrap.scrollTop = distance;
                                    ...
                                    mainWrap.scrollTop = total;reply
                                }
                            }
                        } else {
                            this.user_comment = "";
        ...
        reply(commentId,replayId){  
            if(this.user){
                if(this.reply_content !=='' && this.reply_content.length < 200){
                    this.$http.post(API.voteToReply,{voting_id:this.activityId,company_id:this.companyId,comment_id:commentId,replay_id:replayId,userid:this.user.user_id || 0,comments:this.reply_content})
                    .then((response)=>{
                        if(response.data.code && response.data.code=="200"){
                            //一级回复
                            let newReply = {
        ...
        giveThumbsUp(commentId){
            if(this.user){
                this.$http.post(API.voteToThumbsUp,{comment_id:commentId,userid:this.user.user_id})
                .then((response)=>{
                    if (response.data.code && response.data.code == '200') {
                        for(var i=0;i<this.comDetail.comments.length;i++){
